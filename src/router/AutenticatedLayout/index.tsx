...
const useSidebarExpansion = (): UseSidebarExpansionReturn => {
  ...
  const isCollapsedByBreakpoint = (width ?? Number.POSITIVE_INFINITY) < SIDEBAR_COLLAPSE_BREAKPOINT_PX
...
const SidebarToggleButton = ({ isVisible, isExpanded, onToggle }: SidebarToggleButtonProps): ReactElement => {
  if (!isVisible) {
    return <></>
  }
...
const Sidebar = ({
  ...
  return (
    ...
    >
      ...
      <nav>
        {menuItems.map((item) => {
          const isActive = item.key === "garages" ? pathname === ROUTES.GARAGES : pathname === ROUTES.MENSALISTAS
