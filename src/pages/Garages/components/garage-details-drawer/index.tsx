...
import {
  GarageDetailsHeader,
  GarageDetailTitle,
} from "./garage-details-header";
import {
  GarageDetailsSidebar,
  type GarageDetailsSection,
} from "./garage-details-sidebar";
...
const drawerTabs: TabItem<GarageDetailsTab>[] = [
  { value: "digital", label: "Mensalista Digital" },
];
...
const SectionPlaceholder = ({ title }: { title: string }): ReactElement => (
  <section className="flex min-w-0 flex-1 flex-col items-center justify-center rounded-r-lg border border-l-0 border-estapar-border-light bg-estapar-canvas px-6 py-12 text-center">
    <h3 className="text-base font-semibold text-estapar-medium-gray">
      {title}
    </h3>
    <p className="mt-2 max-w-md text-sm text-estapar-muted">
      Esta seção estará disponível em breve.
    </p>
  </section>
);

export const GarageDetailsDrawer = ({
  open,
  garageId,
  onClose,
}: GarageDetailsDrawerProps): ReactElement => {
  const [activeTab, setActiveTab] = useState<GarageDetailsTab>("digital");
  const [activeSection, setActiveSection] =
    useState<GarageDetailsSection>("plans");
  ...
  const isGarageLoading =
    open && garageId != null && garageQuery.isFetching;

  const availableSpaces =
    (garage?.countSpaces ?? 0) - (garage?.occupiedSpaces ?? 0);

  const garageDeepLink = useMemo(
    () => (garageId ? buildGarageDeepLink(garageId) : ""),
    [garageId],
  );
  ...
  const renderBody = (): ReactElement | null => {
    ...
    if (isGarageLoading) {
      return (
        <p className="p-6 text-center text-sm text-estapar-muted">
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <Tabs
          ...
          aria-label="Categorias da garagem"
        />
        <div className="flex flex-col gap-4 sm:flex-row items-center sm:justify-between sm:gap-6">
          ...
          <QrCode
            size={86}
            value={garageDeepLink}
            className="max-sm:-order-1"
            title={`Abrir garagem ${garage.name}`}
          />
        </div>
        <div className="flex flex-col sm:flex-row sm:items-start">
          ...
          />
          {activeSection === "plans" ? (
            <GaragePlansSection garageId={garageId} />
          ) : null}
          {activeSection === "discounts" ? (
            <SectionPlaceholder title="Descontos" />
          ) : null}
          {activeSection === "settings" ? (
            <SectionPlaceholder title="Configurações" />
          ) : null}
        </div>
      </div>
    );
