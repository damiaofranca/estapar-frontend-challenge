...
const estimateFrozenWidthPx = (width: string | undefined): number => parseWidthToPx(width) ?? FROZEN_FALLBACK_WIDTH_PX
...
const computeStickyOffsets = (columns: readonly Pick<TableColumn<unknown>, "width" | "frozen">[]): StickyOffsets => {
  const n = columns.length
  const leftPx: (number | undefined)[] = Array.from({ length: n }, () => undefined)
  let accLeft = 0
  for (let i = 0; i < n; i++) {
    const col = columns[i]
    if (col?.frozen === "start") {
  ...
  const rightPx: (number | undefined)[] = Array.from({ length: n }, () => undefined)
  let accRight = 0
  for (let i = n - 1; i >= 0; i--) {
    const col = columns[i]
    if (col?.frozen === "end") {
...
const getFrozenCellClassName = (column: Pick<TableColumn<unknown>, "frozen">, kind: FrozenCellKind): string => {
...
const getFrozenCellStyle = (colIndex: number, sticky: StickyOffsets): CSSProperties | undefined => {
...
export const Table = <T,>({
  ...
  const handleRowKeyDown = (event: KeyboardEvent<HTMLTableRowElement>, row: T, rowIndex: number): void => {
    if (!isInteractive) {
      return
    }
  ...
  return (
    ...
    >
      <div className="overflow-x-auto">
        <table className={cn("w-full min-w-[720px] table-fixed text-sm", columnsClassName)}>
          <thead>
          ...
          <tbody>
            {isEmpty ? (
              <tr>
                <td colSpan={columns.length} className="px-4 py-8 text-center text-sm text-estapar-muted">
                  {emptyMessage}
                </td>
              </tr>
            ) : (
              data.map((row, rowIndex) => (
                <tr
                  ...
                  onKeyDown={isInteractive ? (event) => handleRowKeyDown(event, row, rowIndex) : undefined}
                  className={cn(
                  ...
                  {columns.map((column, colIndex) => (
                    <td
                      ...
                      className={cn(
                        ...
                        isInteractive && "group-hover:bg-estapar-muted-surface-alt",
                      )}
                    >
