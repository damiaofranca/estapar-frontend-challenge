...
const isAuthenticateRequest = (config: InternalAxiosRequestConfig | undefined): boolean => {
...
export const api = {
  get: <T>(url: string, config?: AxiosRequestConfig): Promise<T> => axiosInstance.get<T, T>(url, config),
  ...
  delete: <T>(url: string, config?: AxiosRequestConfig): Promise<T> => axiosInstance.delete<T, T>(url, config),
}
